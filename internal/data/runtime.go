...
// UnmarshalJSON implements the interface so that it satisfies the json.Unmarshaler interface. IMPORTANT: Because
// UnmarshalJSON() needs to modify the receiver (our Runtime type), we must use a pointer receiver for this to work
...
func (r *Runtime) UnmarshalJSON(jsonValue []byte) error {
	...
	// Sanity check the parts of the strings to make sure it was in the expected format.
	...
	// the * operator to deference the receiver (which is a pointer to a Runtime type) in order
