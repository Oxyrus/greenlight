...
// the server to listen on, and the name of the current operating environment for the
...
func main() {
	...
	cfg.db = struct {
		...
		dsn: "postgres://" + os.Getenv("DB_USER") + ":" + os.Getenv("DB_PASS") + "@" + os.Getenv("DB_HOST") + "/" + os.Getenv("DB_NAME") + "?sslmode=disable",
	}
	...
	// Defer call to db.Close() so that the connection pool is closed
...
func openDB(cfg config) (*sql.DB, error) {
	...
	// than or equial to 0 will mean that connections are not closed due to their idle time.
