...
import (
	...
	"github.com/oxyrus/greenlight/internal/validator"
)
...
func (app *application) readJSON(w http.ResponseWriter, r *http.Request, dst any) error {
	...
	if err != nil {
		...
		case errors.Is(err, io.EOF):
			return errors.Join(err, errors.New("body must not be empty"))
		...
		// We check for this, extract the field name from the error,
		// and interpolate it into our custom error message.
		case strings.HasPrefix(err.Error(), "json: unknown field "):
			fieldName := strings.TrimPrefix(err.Error(), "json: unknown field ")
			return fmt.Errorf("body contains unknown key %s", fieldName)

		// Use errors.As() function to check whether the error has the type
		// *http.MaxBytesError. If it does, then it means the request body exceeded
		// our size limit of 1MB and we return a clear error message.
...
// interger before returning. If no matching key could be found it returns the provided
